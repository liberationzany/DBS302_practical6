...
4. `app_user` test showing successful `SET session:user123 data`.
...
1. `mongo/init/01-security.js` showing `myAppRole` and `appUser`.
...
4. `appUser` successful insert/find on `myapp.customers`.
5. `appUser` denied access to `admin.system.users`.
