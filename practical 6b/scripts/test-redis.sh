...
docker compose exec redis redis-cli --tls --cacert /certs/redis/ca.crt \
  -u rediss://app_user:appStrongPwd@127.0.0.1:6379 SET session6b:user123 data
docker compose exec redis redis-cli --tls --cacert /certs/redis/ca.crt \
  -u rediss://app_user:appStrongPwd@127.0.0.1:6379 GET session6b:user123
...
set +e
docker compose exec redis redis-cli --tls --cacert /certs/redis/ca.crt \
  -u rediss://monitoring:monitorPwd@127.0.0.1:6379 SET session6b:monitor nope
set -e
